     ...
     String input=m_jCovers.getText();
       ...
       }else if(pattern.matcher(input).matches())
      {
       ...
       if(restaurantMap.getStatus()==true){
        ...
        try {
            localDlReceipts.insertTableCovers(UUID.randomUUID().toString(), place.getId(), Integer.parseInt(m_jCovers.getText()),m_panelticket.getActiveTicket().getSplitSharedId());
        } catch (BasicException ex) {
            Logger.getLogger(JTableCover.class.getName()).log(Level.SEVERE, null, ex);
        }
        }else{
            localDlReceipts.updateNoOfCovers(place.getId(), Integer.parseInt(m_jCovers.getText()),m_panelticket.getActiveTicket().getSplitSharedId());
//              int noOfCovers = 0;
//            try {
